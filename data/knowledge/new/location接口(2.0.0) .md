...
本文档面向外部联调人员，整理 `location` 在 ROS2 中实际对外暴露的接口。当前代码实际注册了 2 个 Topic 和 2 个 Service。

## Topic 总览

|话题名|消息类型|说明|Qos|
|---|---|---|---|
|`/zj_humanoid/navigation/odom_info`|`nav_msgs/msg/Odometry`|发布地图坐标系下的定位结果，`frame_id` 为 `map`，`child_frame_id` 为 `body_norm`。|默认|
|`/zj_humanoid/perception/location_code`|`module_common_msgs/msg/ModuleStatus`|发布定位模块状态和错误码，仅在状态序列变化时更新。|默认|

## Service 总览

|服务名|消息类型|说明|Qos|
|---|---|---|---|
|`/zj_humanoid/perception/reloc`|`naviai_localization_msgs/srv/Lio`|启动一次定位/重定位任务，支持 `config`、`auto`、`lio_only` 三种模式。|默认|
|`/zj_humanoid/perception/location_version`|`std_srvs/srv/Trigger`|查询算法包、ROS 包和中间件版本信息，返回 JSON 字符串。|默认|
...
- 消息类型：`naviai_localization_msgs/srv/Lio`
...
```Plain Text
naviai_localization_msgs/srv/Lio
...
```Bash
ros2 service call /zj_humanoid/perception/reloc naviai_localization_msgs/srv/Lio "{method: auto, map_path: D4C, x_pos: 0.0, y_pos: 0.0, z_pos: 0.0, x_ori: 0.0, y_ori: 0.0, z_ori: 0.0, w_ori: 1.0}"
```
